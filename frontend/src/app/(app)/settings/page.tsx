...
export default function SettingsPage() {
  const { user, profile, plan, refreshProfile } = useAuthContext();
  ...
  useEffect(() => {
    ...
    if (user) {
      supabase.from("user_api_keys").select("*").eq("user_id", user.id).then(({ data }) => {
        if (data) {
          for (const key of data as Array<{ engine: string; api_key_encrypted: string | null }>) {
            if (key.engine === "openai") setOpenaiKey(key.api_key_encrypted ? "••••••••" : "");
            if (key.engine === "deepl") setDeeplKey(key.api_key_encrypted ? "••••••••" : "");
            if (key.engine === "gemini") setGeminiKey(key.api_key_encrypted ? "••••••••" : "");
          }
        }
      });
  ...
  const handleSaveApiKeys = async () => {
    ...
    setSavingKeys(true);
    const engines = [
      { engine: "openai", api_key: openaiKey },
      { engine: "deepl", api_key: deeplKey },
      { engine: "gemini", api_key: geminiKey },
    ];
    for (const { engine, api_key } of engines) {
      if (api_key && !api_key.startsWith("••")) {
        const { data: existing } = await supabase.from("user_api_keys").select("id").eq("user_id", user.id).eq("engine", engine).maybeSingle();
        if (existing) {
          await supabase.from("user_api_keys").update({ api_key_encrypted: api_key, updated_at: new Date().toISOString() }).eq("id", existing.id);
        } else {
          await supabase.from("user_api_keys").insert({ user_id: user.id, engine, api_key_encrypted: api_key });
        }
      }
    }
    setSavingKeys(false);
  ...
  const handleSaveStorage = async () => {
    ...
    try {
      ...
      if (storageConfigId) {
        ...
        await supabase.from("user_storage_configs").update(updatePayload).eq("id", storageConfigId);
      } else {
        const { data } = await supabase.from("user_storage_configs").insert(base).select("id").single();
  ...
  const handleDeleteStorage = async () => {
    ...
    try {
      ...
      setB2KeyId(""); setB2AppKey(""); setB2BucketName(""); setB2BucketId("");
        ...
        <TabsContent value="api-keys" className="mt-6 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Çeviri motorlarının API anahtarlarını buraya girin. Anahtarlar şifreli olarak saklanır.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label className="flex items-center gap-2">OpenAI API Key<Badge variant="secondary" className="text-[10px]">GPT-4</Badge></Label>
                  <a href="https://platform.openai.com/api-keys" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-primary hover:underline">Anahtar Al<ExternalLink className="h-3 w-3" /></a>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label className="flex items-center gap-2">DeepL API Key<Badge variant="secondary" className="text-[10px]">Pro</Badge></Label>
                  <a href="https://www.deepl.com/pro-api" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-primary hover:underline">Anahtar Al<ExternalLink className="h-3 w-3" /></a>
                </div>
                <Input type="password" placeholder="..." value={deeplKey} onChange={(e) => setDeeplKey(e.target.value)} />
              </div>
              <Separator />
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label className="flex items-center gap-2">Google Gemini API Key<Badge variant="secondary" className="text-[10px]">Gemini</Badge></Label>
                  <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-primary hover:underline">Anahtar Al<ExternalLink className="h-3 w-3" /></a>
                </div>
                <Input type="password" placeholder="..." value={geminiKey} onChange={(e) => setGeminiKey(e.target.value)} />
              </div>
